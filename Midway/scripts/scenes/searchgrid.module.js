    ...
    return {
        ...
        highlightArrivalZones: function (side) {
            var topZone = side == "IJN" ? "A1A" : "I1B",
                ...
                bottomCoords = addVectors(this.zoneToTopLeftCoords(bottomZone), { x: zonesize, y: zonesize }),
                width = bottomCoords.x - topCoords.x,
                height = bottomCoords.y - topCoords.y;

            this.restImg = ctx.getImageData(topCoords.x, topCoords.y, width, height);
            ctx.save();
            ctx.globalAlpha = 0.2;
            ctx.fillStyle = "#ffd651";
        ...
        removeArrivalZones: function (side) {
            ...
            ctx.putImageData(this.restImg, topCoords.x, topCoords.y);
        },
        /*-------------------------------------------------------------------*/
        ...
        drawSelector: function (topLeft, sizeInZones) {
            ...
            if (sizeInZones == 1)
                ...
                this.restImg = ctx.getImageData(left, top, sideLength + 3, sideLength + 3);
            ctx.save();
            ctx.globalAlpha = 0.6;
            ctx.lineWidth = 4;
        ...
        removeSelector: function (left, top) {
            ctx.putImageData(this.restImg, left, top);
        },
        /*-------------------------------------------------------------------*/
